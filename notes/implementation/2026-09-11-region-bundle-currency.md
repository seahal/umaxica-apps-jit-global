...
- Related decisions/docs: `docs/architecture/preference.md` "Region is a regional-bundle reset",
  ...
  `notes/implementation/2026-09-09-ux-preference-asset-hostname-fixes.md`.
...
- Decision: add `currency` to the existing regional-bundle reset (`REGIONAL_DEFAULT_OPTION_NAMES`
  ...
  - Alternatives considered: request-local `?cu` seeding on bootstrap only — rejected; the report is
    a `/preference/region` write, and bootstrap already seeds language from `?ri` without rewriting
    the other region-owned fields until that write.
...
- Tests run:
  `bin/rails test test/integration/acme_preference_test.rb test/controllers/concerns/preference/core_test.rb test/controllers/concerns/preference_write_authorization_refusal_test.rb test/controllers/concerns/preference/base_test.rb`
  — 260 runs, 0 failures.
