...
> canonical Identity / Persona / Organization authority. `Member`, `ClientMembership`, and the
> legacy `OperatorOrganization` are Global; the decomposition this document audits
...
This document audits the ambiguous placement cluster around `Member`, `ClientMembership`, and the
legacy `OperatorOrganization` resource.
...
- `zenith` is the target canonical placement for Principal / Identity / Persona / Organization
  authority.
...
| --------------------------------------------------------------------------------------------------- | ----------------------------------------------------- | ------------------------------------------------------------- | ----------------------- | -------------------- | --------------------------------------------------------------------------------------------- | ---------------------------------------------------------------------------------------------------------------------------------------------------- |
| `Member`                                                                                            | `app/models/member.rb`                                | `members`                                                     | `app_principal`         | `AppPrincipalRecord` | account-like transitional row                                                                 | Belongs to `Client` through `user`; links to Avatar governance and client-member history.                                                            |
...
| `OperatorOrganization`                                                                              | `app/models/operator_organization.rb`                 | `organizations`                                               | `org_zenith`            | `OrgPrincipalRecord` | legacy org-principal hierarchy container                                                      | Concrete legacy model mapped to the `organizations` table; has `domain`, `name`, status, parent, department, and operator columns.                   |
...
| `ClientPersona`                                                                                     | `app/models/client_persona.rb`                        | `personas`                                                    | `app_zenith`            | `AppRpRecord`        | Persona authority                                                                             | Includes the common `Persona` interface; belongs to `ClientIdentity`; has assignments and memberships.                                               |
| `Agent`                                                                                             | `app/models/agent.rb`                                 | `agents`                                                      | `org_zenith`            | `OrgRpRecord`        | Persona authority                                                                             | Includes the common `Persona` interface; belongs to `OperatorIdentity`; has assignments and memberships.                                             |
| `Individual`                                                                                        | `app/models/individual.rb`                            | `individuals`                                                 | `com_zenith`            | `ComRpRecord`        | Persona authority                                                                             | Includes the common `Persona` interface; belongs to `VisitorIdentity`; has assignments and memberships.                                              |
...
| `PersonaMembership`                                                                                 | `app/models/persona_membership.rb`                    | `persona_memberships`                                         | `app_zenith`            | `AppRpRecord`        | membership                                                                                    | Persona-to-enterprise/unit membership with grant, approval, revoke, kind, state, and active-primary semantics.                                       |
| `AgentMembership`                                                                                   | `app/models/agent_membership.rb`                      | `agent_memberships`                                           | `org_zenith`            | `OrgRpRecord`        | membership                                                                                    | Persona-to-bureau/unit membership with grant, approval, revoke, kind, state, and active-primary semantics.                                           |
| `IndividualMembership`                                                                              | `app/models/individual_membership.rb`                 | `individual_memberships`                                      | `com_zenith`            | `ComRpRecord`        | membership                                                                                    | Persona-to-company/unit membership with grant, approval, revoke, kind, state, and active-primary semantics.                                          |
| `PersonaAssignment`                                                                                 | `app/models/persona_assignment.rb`                    | `persona_assignments`                                         | `app_zenith`            | `AppRpRecord`        | assignment                                                                                    | Identity-to-Persona access assignment with assigned/revoked timestamps.                                                                              |
| `AgentAssignment`                                                                                   | `app/models/agent_assignment.rb`                      | `agent_assignments`                                           | `org_zenith`            | `OrgRpRecord`        | assignment                                                                                    | Identity-to-Persona access assignment with assigned/revoked timestamps.                                                                              |
| `IndividualAssignment`                                                                              | `app/models/individual_assignment.rb`                 | `individual_assignments`                                      | `com_zenith`            | `ComRpRecord`        | assignment                                                                                    | Identity-to-Persona access assignment with assigned/revoked timestamps.                                                                              |
| `OperatorWorkspaceAccount`                                                                          | `app/models/operator_workspace_account.rb`            | `operator_workspace_accounts`                                 | `org_zenith`            | `OrgRpRecord`        | legacy account-like transitional row                                                          | Belongs to `Operator` and `Department`; overlaps with the `Agent` direction.                                                                         |
...
| Account-like behavior                   | Carries account-like fields but does not include the common `Persona` interface.                                                                                     | `member.rb` associations and schema.                                  | It is not automatically the target canonical Persona.                                   |
| Overlap with `ClientPersona`            | Both carry moniker/public-id shape. `ClientPersona` is in `app_zenith` and belongs to `ClientIdentity`; `Member` is in `app_principal` and belongs to `Client`.      | `member.rb`, `client_persona.rb`, dictionary definition.              | `Member` overlaps with the canonical app Persona direction but remains transitional.    |
...
| Authority-suggesting columns            | `public_id`, `moniker`, and status.                                                                                                                                  | Schema comment and model.                                             | These suggest account-authority semantics, but they are mixed with legacy bridge links. |
...
## OperatorOrganization (legacy resource)

| aspect                                           | finding                                                                                                                                 | evidence                                                                       | implication                                                                                |
| ------------------------------------------------ | --------------------------------------------------------------------------------------------------------------------------------------- | ------------------------------------------------------------------------------ | ------------------------------------------------------------------------------------------ |
| Current database                                 | `org_zenith`                                                                                                                            | Schema comment and `OperatorOrganization < OrgPrincipalRecord`.                | Current storage is org-zenith, not proof of canonical authority ownership.                 |
| Base class                                       | `OrgPrincipalRecord`                                                                                                                    | Model inheritance.                                                             | The legacy model remains separate from the common `Organization` interface.                |
| Current representation                           | Org-principal hierarchy container with `domain`, `name`, parent/status/operator/department columns.                                     | `operator_organization.rb` schema and validations.                             | It has organization-authority shape and legacy operational-container history.              |
| Relationships                                    | Has many `Division` rows and has many `Department` rows as `workspace`.                                                                 | `operator_organization.rb`.                                                    | It overlaps legacy workspace/division/department naming.                                   |
...
| Credential/session/lifecycle coupling            | No direct credential/session associations observed in `OperatorOrganization`; status exists.                                            | `operator_organization.rb`.                                                    | Credential/session rows are not part of this model, but lifecycle/status semantics remain. |
| Not concluded                                    | Whether this legacy `OperatorOrganization` maps to `Bureau`, remains a retained application container, or is decomposed is not decided. | Current docs identify it as ambiguity zone.                                    | Later ADR required before movement.                                                        |
...
- reason: `OperatorOrganization` has authority-like organization columns and hierarchy links, but
  its legacy org-principal workspace/container role must be separated from canonical zenith
  organization authority before migration.
...
automatically part of Principal / Identity / Persona / Organization authority placement. They may
...
## Relationship to Zenith Authority Graph

| principal-side item                  | zenith-side related item                           | overlap                                                                    | difference                                                                                                                                                   | likely direction                                                                                                          |
| ------------------------------------ | -------------------------------------------------- | -------------------------------------------------------------------------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------ | ------------------------------------------------------------------------------------------------------------------------- |
| `Member`                             | `ClientPersona`                                    | Both are app account-like rows and carry public identity fields.           | `ClientPersona` belongs to `ClientIdentity` in `app_zenith`; `Member` belongs to `Client` in `app_principal` and links to legacy Avatar/client-member rows.  | Decompose or map the account-authority portion toward `ClientPersona`; retain/retire compatibility portions deliberately. |
| `ClientMembership`                   | `PersonaMembership`                                | Both are membership-shaped.                                                | `PersonaMembership` joins a Persona implementation to `Enterprise`/unit with grant/revoke metadata; `ClientMembership` joins `Client` to raw `workspace_id`. | Decide whether it maps to zenith membership, regional membership, or bridge state.                                        |
| `ClientMembership`                   | `PersonaAssignment`                                | Both involve account access graph.                                         | `PersonaAssignment` assigns `ClientIdentity` to `ClientPersona`; `ClientMembership` is actor/workspace membership.                                           | Do not treat as assignment without a mapping decision.                                                                    |
| `OperatorOrganization`               | `Bureau` / `BureauUnit`                            | Both represent org-side organization hierarchy.                            | `OperatorOrganization` is a legacy org-principal workspace/container shape; `Bureau` is the org-zenith canonical hierarchy root with units/closures.         | Candidate mapping toward `Bureau` authority after decomposition.                                                          |
| `OperatorOrganization`               | `Enterprise` / `Company`                           | Surface-specific organization roots share some structural vocabulary.      | Surface-specific canonical roots live in each zenith DB; the legacy `OperatorOrganization` remains org-local.                                                | Use as pattern evidence, not direct cross-surface table movement.                                                         |
| `Client`                             | `ClientIdentity`, `ClientAccount`, `ClientPersona` | Runtime actor links to identity binding, RP projection, and Persona graph. | `Client` owns credentials/lifecycle/session-adjacent links; zenith rows separate identity binding/account/projection.                                        | Decompose, then bind authority portions to zenith graph.                                                                  |
| `Operator`                           | `OperatorIdentity`, `OperatorAccount`, `Agent`     | Runtime actor links to identity binding, RP projection, and Persona graph. | `Operator` owns credentials/lifecycle/session-adjacent links; `Agent` is the canonical org Persona implementation.                                           | Decompose, then bind authority portions to zenith graph.                                                                  |
| `Visitor`                            | `VisitorIdentity`, `VisitorAccount`, `Individual`  | Runtime actor links to identity binding, RP projection, and Persona graph. | `Visitor` owns credentials/lifecycle/session-adjacent links; `Individual` is the canonical com Persona implementation.                                       | Decompose, then bind authority portions to zenith graph.                                                                  |
| `OperatorWorkspaceAccount`           | `Agent`                                            | Both are org account-like resource shapes.                                 | `OperatorWorkspaceAccount` is legacy workspace-account shape tied to `Operator` and `Department`; `Agent` belongs to `OperatorIdentity`.                     | Transitional candidate; likely retire/map to `Agent` after explicit decision.                                             |
...
| `OperatorOrganization`                                    | `org_zenith`                                                              | `organization_authority`, `transitional`                            | `unknown`                                                    | `candidate`     | Authority-like organization data overlaps legacy workspace/container role.                 |
...
| `ClientPersona`                                           | `app_zenith`                                                              | `account_authority`                                                 | `zenith`                                                     | `settled`       | Canonical app Persona row; the common `Persona` name is an interface.                      |
| `Agent`                                                   | `org_zenith`                                                              | `account_authority`                                                 | `zenith`                                                     | `settled`       | Canonical org Persona row.                                                                 |
| `Individual`                                              | `com_zenith`                                                              | `account_authority`                                                 | `zenith`                                                     | `settled`       | Canonical com Persona row.                                                                 |
...
| `PersonaMembership`                                       | `app_zenith`                                                              | `membership`                                                        | `zenith`                                                     | `candidate`     | Persona-to-enterprise membership/history shape.                                            |
| `AgentMembership`                                         | `org_zenith`                                                              | `membership`                                                        | `zenith`                                                     | `candidate`     | Persona-to-bureau membership/history shape.                                                |
| `IndividualMembership`                                    | `com_zenith`                                                              | `membership`                                                        | `zenith`                                                     | `candidate`     | Persona-to-company membership/history shape.                                               |
...
- `ClientPersona`, `Agent`, and `Individual` are current zenith Persona authority rows.
...
- `OperatorOrganization`, because it has organization-authority shape and legacy org-principal
...
- Whether `ClientMembership` maps to zenith membership, regional membership, or compatibility
  bridge.
- Whether legacy `OperatorOrganization` maps toward `Bureau` authority or remains as a
  retained/future regional application container.
...
- Do not move a model to `zenith` just because its name sounds like Persona, Identity, Principal, or
  Organization.
...
- [ ] Decide whether `OperatorOrganization` should move toward zenith organization authority or
      become a regional application container.
