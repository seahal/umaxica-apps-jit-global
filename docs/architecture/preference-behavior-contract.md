...
the equivalent non-Core browser endpoints remain in the legacy `/web/v0` namespace until their own
...
(for example a non-Core `web/v0`/`edge/v0/cookies` controller or Core's `api/v0/preferences/cookies`
controller) differs from its sibling surfaces, the difference must be explained by a callback that
genuinely does not exist on that surface's `ApplicationController` (verify before assuming parity —
`skip_before_action` on an undefined callback raises `ArgumentError` at boot). Otherwise treat the
divergence as a defect and bring the surfaces back in line.
